/**
 * B50 分享卡片（v1.13.0）：总分 + 两池合计 + TOP15 缩略（曲绘网格），
...
export function B50ShareCard({ rawData, scores, serverRating, userName, onReady }: Props) {
  ...
  const doShare = async () => {
    await captureAndShare(viewShotRef.current, shareCardFileName('MaiMate-b50'));
  };
  ...
  const renderCell = (entry: (typeof topNew)[number], key: string) => {
    ...
    return (
      <View key={key} style={[styles.cell, { borderColor }]}>
  ...
  return (
    <ViewShot ref={viewShotRef} options={{ format: 'png', quality: 1 }} style={styles.host}>
      <View style={styles.card}>
        ...
        <View style={styles.grid}>{topNew.map((entry, index) => renderCell(entry, `n${index}`))}</View>

        <Text style={styles.sectionTitle}>旧曲 TOP35</Text>
        <View style={styles.grid}>{topOld.map((entry, index) => renderCell(entry, `o${index}`))}</View>
