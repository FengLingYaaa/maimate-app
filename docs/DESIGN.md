...
> 最后更新：2026-09-05
> 当前版本：`v1.17.1`
> 当前状态：Phase A-F 与 v1.16.0–v1.17.1 各轮用户反馈迭代均已实现并云构建发布；v1.17.1（第十七轮用户反馈）功能提交、tag、云构建 run `33950192118` 与 Release（版本化 APK + latest 并存）已完成，落地页已同步部署（见变更记录）；真机验收仍待 Android 设备。
...
### v1.16.6（2026-08-29，第十二轮用户反馈）

- 用户反馈与处置：① 战报「+0 RA（B50）」不再显示——`app/snapshots.tsx` 与 `BattleReportShareCard` 渲染条件改为 `b50Delta > 0`（仅正向，用户确认）；② 存储占用统计错误——曲绘恒 0B/0 张根因是 legacy `getInfoAsync` 不含 size，`storage-usage.ts` 重写为 expo-file-system 新 API（`Directory.list()` + `File.size` 同步属性）；应用本体口径改为双行说明（APK 59.4 MB / 系统显示 ~120 MB 为安装后解压体积）；新增「其它缓存」分项（cache 根目录 covers 之外内容，对应用户看到的 ~100 MB 旧版残留）并支持清理（`clearOtherCache`）；③ 按歌曲 ID 搜索——`getMusicSearchScore` 对纯数字词优先 ID 精确（=1）/前缀（=0.85）匹配，三处 FilterBar 共用生效；④ 导出双选项——`exportScoresCsv(mode)` 与 `exportBackup(mode)`：'save' 走 SAF `requestDirectoryPermissionsAsync` 每次选目录写入（取消抛 `SAF_SAVE_CANCELLED`，UI 显示「已取消保存」），'share' 保持原分享链路；设置页 CSV 行与备份页各为「仅保存…/保存并分享」双按钮（自绘，非 Alert）；⑤ 音频权限——`blockedPermissions` 扩为 RECORD_AUDIO + READ_MEDIA_AUDIO + READ_MEDIA_VIDEO + READ/WRITE_EXTERNAL_STORAGE，APK manifest 不再含任何音频/媒体读权限（保存图片不再触发音频授权弹窗）；⑥ 三模式抽歌——`plan-draw-history.ts` 重写为 v2 结构 `{日期:{模式:{keys,draws,fallback}}}`（旧数据自动迁移为 plan 模式），计划/全曲/条件各自独立记录与防重复，历史弹层加模式筛选 chips + 「重置今日」；⑦ 回落提示修复——记录加 `fallback` 持久化标记，计划池出现新键时自动清除（`clearFallbackFlag`）提示消失；⑧ 计数改按次数——`draws` 每抽 +1（抽到重复也算），键集去重另存；⑨ 拍照识别——× 失效修复（`onStartShouldSetResponder` + stopPropagation + rAF）；匹配放宽（Levenshtein 距离 1 且题长 ≥3 入围 0.72 档，解决缺一个字母不显示）；结果上限 8 → 12；一图多曲天然支持（逐行匹配）；逐图进度文案已有（第 i/N 张）；⑩ 清空计划提速——`bulkRemoveEntries` 全量清空分支直接置空 + 立即持久化 `[]`（跳过 normalize 大列表 diff）。更新日志浮层补 1.16.6 条目。
- 回归与版本：七组门禁全绿；`expo-doctor` 21/21；`expo export --platform android` 通过；`app.json` 升 `1.16.6`/versionCode `32`。发布补记（当日）：功能提交 `c921c43`（16 文件 +482/−160）；轻量标签 `v1.16.6` git 推送一次成功。云构建 run `33259266400` 一次通过。Release 为 <https://github.com/FengLingYaaa/maimate-app/releases/tag/v1.16.6>；资产 `MaiMate-latest.apk` 大小 `62,310,900` bytes（59.4 MB），SHA-256 `7dd265d28ac99cd91c086e54f41dee3f87217e804a3f55b23134298a888b0e9c`（本地下载资产复算）。落地页更新 v1.16.6 文案、日志与 SHA 后经 wrangler 部署 Pages（deployment `d44523cc.maimate-landing.pages.dev`）；线上校验落地页含 v1.16.6 与新 SHA、GitHub 直链 200 / content-length `62,310,900` 与资产一致。SHA/大小回填为文档收尾提交。等待用户真机验收（存储占用曲绘/其它缓存数值与清理、纯数字 ID 搜索、导出「仅保存」SAF 目录选择、保存图片不再弹音频授权、三模式抽歌历史/计数/回落提示消除、识别删图按钮与缺字母匹配、清空计划速度）。

### v1.16.7（2026-09-02，第十三轮用户反馈）

- 用户反馈与处置：① 详情页展示数字 ID——谱师行下新增「ID」行（`music.id`）；② 曲绘缓存误入「其它缓存」——根因是 `measureOtherCache`/`clearOtherCache` 用 `item.name === 'covers'` 等值比较漏判（部分平台 name 含路径成分），改为 `item.uri.endsWith('/covers')` 判定；`clearCoverCache` 同时删除目录本身，避免目录残留恒被误计；③ CSV 导出独立次级页——新增 `settings/export-csv.tsx`（说明 + 仅保存/保存并分享双按钮 + 结果消息区），设置栈注册 `export-csv` 路由（route-check 期望列表同步更新），设置页行改为跳转入口；④ 备份页「仅保存…」删省略号，统一文案「仅保存」；⑤ 识别删除单张卡顿——重构为**增量匹配**：`perImageMatches: Map<uri, TitleMatch[]>`，删图=删该图候选+重新合并（`mergeImageMatches` 去重取最优、排序、截断 12），不再全量重扫；加图=只匹配该图（`matchSongTitlesForImage`，不截断防弱匹配误杀）；与全量一次性匹配数学等价（原实现即取每曲全文本最优），输出一致；`matchSongTitles` 保留为兼容包装（回归脚本零改动）；⑥ OCR 原始文字默认折叠（「查看识别文字（N 张）」点击展开）；⑦ 确认式搜索——FilterBar 键入不再自动检索（删除 180ms 防抖），新增「搜索」按钮 + 键盘 search 键触发 `runSearch` 分片匹配（曲库 80/片、计划 60/片，`setImmediate` 让出线程）并显示「匹配中 i/N 首…」，完成后 onApply；历史 chip 回填只改草稿；曲库与计划各自实现 runSearch（计划复用 matchesPlanRow 保持计划难度口径）；⑧ 快照对比战报行点击跳详情（source: 'snapshots'，带 difficultyIndex），详情页 handleBack 补 snapshots 分支 `replace('/snapshots')`。更新日志浮层补 1.16.7 条目。
- 回归与版本：七组门禁全绿（route-check 期望列表补 export-csv）；`expo-doctor` 21/21；`expo export --platform android` 通过；`app.json` 升 `1.16.7`/versionCode `33`。发布补记（当日）：功能提交 `8b0c80a`（17 文件 +339/−103）；轻量标签 `v1.16.7` git 推送一次成功。云构建 run `33290637037` 一次通过。Release 为 <https://github.com/FengLingYaaa/maimate-app/releases/tag/v1.16.7>；资产 `MaiMate-latest.apk` 大小 `62,318,660` bytes（59.4 MB），SHA-256 `392374db61a6ac47443b815d43de5f9eb41f3fd61a73248a6c98ca58ec3cc95d`（本地下载资产复算）。落地页更新 v1.16.7 文案、日志与 SHA 后经 wrangler 部署 Pages（deployment `9bd1d29f.maimate-landing.pages.dev`）；线上校验落地页含 v1.16.7 与新 SHA、GitHub 直链 200 / content-length `62,318,660` 与资产一致。SHA/大小回填为文档收尾提交。等待用户真机验收（详情页 ID 行、存储页曲绘/其它缓存分项正确分离、CSV 导出次级页、备份按钮文案、识别删图不再卡顿、OCR 文字折叠、确认式搜索与匹配进度、快照行跳详情返回回快照页）。

### v1.16.8（2026-08-30，第十四轮用户反馈）

- 用户反馈与处置（排查先行）：① 曲绘缓存仍进「其它缓存」——解包 APK 证实应用零内置曲绘，根因是 `CoverImage` 旧实现「先显示远端图、缓存后台落盘」，每张曲绘必然先被系统图片库（Fresco）按远端 URL 缓存一份进 cache 根目录；修复为**本地优先**（占位图 → 等本地缓存 → 有则显示本地，远端只做下载源，绝不直连显示），并给「其它缓存」行加**明细子项**（cache 根目录一级子项名字+字节，取前 3 项），`clearOtherCache` 追加 `Image.clearDiskCache/clearMemoryCache` 清系统图片缓存；② `11809` 搜出 1 条排查为**正确行为**（API 实测 11809=DX 曲 `Åntinomiε`，纯数字=按 ID 精确命中），卡顿为真 bug——v1.16.7 确认式搜索评分分片后 `applyFilters` 又全量 filter+sort 一遍（双遍劳动）；修复：`getMusicSearchScore` 纯数字查询**短路**（只做 ID 精确=1/前缀=0.85/≤3 位包含=0.5 封顶，跳过全部模糊评分），store 新增 `applyFiltersChunked`（评分+过滤一趟分片 80/片 `setImmediate` 让出，`sortMusicItems` 导出复用，排序只对命中子集），FilterBar 进度从文本改为**真实进度条**（3px accent 填充条 0→100%，完成后短暂显示「匹配完成 · N 首」）；③ 成绩图噪音（39/BREAK 等）——采用「整行噪音判定 + 曲名豁免」双层：规范化后整行等于停用词（PERFECT/BREAK/COMBO/SCORE/百分比/纯数字等）的行不参与匹配，但被任一真实曲名包含的行豁免（《Break!Break!Break!》规范化为 breakbreakbreak 天然不命中整行规则），≤3 位纯数字撞标题封顶 0.5 低于 ID 命中；④ 曲绘分级缓存——新依赖 `expo-image-manipulator@~57.0.14`（先误装 14.0.8 触发 doctor 版本检查失败，按 bundledNativeModules 对齐 SDK 57 统一版本后 21/21 恢复），`cover-cache.ts` 重写：`covers/thumbs/<id>-<hash>.jpg`（512px JPEG 80，下载后本地压缩、**原图不落盘**，压缩失败回退原图改名）+ `covers/full/<id>-<hash>.png`（详情页 `preferFull` 按需，LRU 上限 80 张 AsyncStorage 记录 last-used，`cover-lru.ts` 实现 touch/evict/clear），曲绘总体积从无上限变为「缩略图 ~40MB 顶 + 原图 80 张」双上限；`CoverImage` 增 `preferFull` prop（详情页 hero 与看图模式传 true），`resolveCoverCacheUri` 语义改为「本地优先、绝不回退远端 URL」；⑤ 顺手补 v1.16.7 遗留：识别在途竞态守卫（`applyImageResult` 落位前检查 uri 仍在 imageUris，删除后的迟到结果丢弃）。更新日志浮层补 1.16.8 条目。
- 回归与版本：七组门禁全绿；`expo-doctor` 21/21（修复依赖版本线后）；`expo export --platform android` 通过；`app.json` 升 `1.16.8`/versionCode `34`。发布补记（当日）：功能提交 `9a37a6c`（17 文件 +373/−97）；轻量标签 `v1.16.8` git 推送一次成功。云构建 run `33296836362` 一次通过。Release 为 <https://github.com/FengLingYaaa/maimate-app/releases/tag/v1.16.8>；资产 `MaiMate-latest.apk` 大小 `62,356,656` bytes（59.5 MB），SHA-256 `3b214c0be8b2cf95cd90709b46aa45462ef0860a1d29f41830b6fb4f65d45cd1`（本地下载资产复算）。落地页更新 v1.16.8 文案、日志与 SHA 后经 wrangler 部署 Pages（deployment `8fd33b9c.maimate-landing.pages.dev`）；线上校验落地页含 v1.16.8 与新 SHA、GitHub 直链 200 / content-length `62,356,656` 与资产一致。SHA/大小回填为文档收尾提交。等待用户真机验收（其它缓存明细子项是否点名系统图片缓存、清理后曲绘不再回流、11809 搜索毫秒级且仅 1 条、搜索进度条推进、成绩图识别 BREAK/39 行不再干扰、列表曲绘为缩略图/详情页原图清晰）。
- 发布补记（当日）：功能提交 `c921c43`（16 文件 +482/−160）；轻量标签 `v1.16.6` git 推送一次成功。云构建 run `33259266400` 一次通过。Release 为 <https://github.com/FengLingYaaa/maimate-app/releases/tag/v1.16.6>；资产 `MaiMate-latest.apk` 大小 `62,310,900` bytes（59.4 MB），SHA-256 `7dd265d28ac99cd91c086e54f41dee3f87217e804a3f55b23134298a888b0e9c`（本地下载资产复算）。落地页更新 v1.16.6 文案、日志与 SHA 后经 wrangler 部署 Pages（deployment `d44523cc.maimate-landing.pages.dev`）；线上校验落地页含 v1.16.6 与新 SHA、GitHub 直链 200 / content-length `62,310,900` 与资产一致。SHA/大小回填为文档收尾提交。等待用户真机验收（存储占用曲绘/其它缓存数值与清理、纯数字 ID 搜索、导出「仅保存」SAF 目录选择、保存图片不再弹音频授权、三模式抽歌历史/计数/回落提示消除、识别删图按钮与缺字母匹配、清空计划速度）。
- 发布补记（当日）：功能提交 `2c8a780`（13 文件 +313/−163）；轻量标签 `v1.16.5` git 推送一次成功。云构建 run `33249234129` 一次通过。Release 为 <https://github.com/FengLingYaaa/maimate-app/releases/tag/v1.16.5>；资产 `MaiMate-latest.apk` 大小 `62,258,012` bytes（59.4 MB），SHA-256 `1d9a679b002e9b0290d7aa61de915db92b3cb88f022ba92be7334e0646b28e3e`（本地下载资产复算）。落地页更新 v1.16.5 文案、日志与 SHA 后经 wrangler 部署 Pages（deployment `6cd1e0ea.maimate-landing.pages.dev`）；线上校验落地页含 v1.16.5 与新 SHA、GitHub 直链 200 / content-length `62,258,012` 与资产一致。SHA/大小回填为文档收尾提交。等待用户真机验收（计划长列表滑动与「到底」不再卡顿闪退、战报 RA（B50）为真实总分影响、搜索历史管理模式、存储占用三项与清理、运势曲绘完整正方形）。
- 发布补记（当日）：功能提交 `8e6ba6d`（9 文件 +176/−6）；轻量标签 `v1.16.4` git 推送一次成功。云构建 run `33243998179` 一次通过。Release 为 <https://github.com/FengLingYaaa/maimate-app/releases/tag/v1.16.4>；资产 `MaiMate-latest.apk` 大小 `62,250,492` bytes（59.4 MB），SHA-256 `febe9869970eaea67ab958b2ba85056b5b55339bc068014c73ef6c101bd5fd25`（本地下载资产复算）。落地页更新 v1.16.4 文案、日志与 SHA 后经 wrangler 部署 Pages（deployment `4fd06512.maimate-landing.pages.dev`，部署后边缘传播约 20 秒）；线上校验落地页含 v1.16.4 与新 SHA、下载按钮 GitHub 直链有效、GitHub 直链 200 / content-length `62,250,492` 与资产一致。SHA/大小回填为文档收尾提交。等待用户真机验收（快照页标题/返回键/状态栏不再遮挡、战报榜内曲目显示 +N RA（B50）、搜索历史长按删除、详情页成绩曲线（多快照后更有意义））。
- 发布补记（当日）：功能提交 `f20a4e2`（14 文件 +113/−56）；轻量标签 `v1.16.3` git 推送一次成功。云构建 run `33241108334` 一次通过。Release 为 <https://github.com/FengLingYaaa/maimate-app/releases/tag/v1.16.3>；资产 `MaiMate-latest.apk` 大小 `62,246,388` bytes（59.4 MB），SHA-256 `a784260fce6035e7565c8cfc64f23af0fbe8009f6ae3021cc045a248c05c85d5`（本地下载资产复算）。落地页更新 v1.16.3 文案、日志、SHA 与 GitHub 直链下载按钮（探测脚本已删）后经 wrangler 部署 Pages（deployment `fad35420.maimate-landing.pages.dev`）；线上校验落地页含 v1.16.3 与新 SHA、下载按钮 href 为 `releases/latest/download/MaiMate-latest.apk` 直链、页面无 fetch 探测代码、GitHub 直链 200 / content-length `62,246,388` 与资产一致。SHA/大小回填为文档收尾提交。等待用户真机验收（快照管理从信息查询进入为独立页面且不再跳设置、战报 RA（B50）口径、到底按钮右下角、计划卡紧凑、计划查漏条、下载站直链在国内的连通性）。


### v1.16.9（2026-09-03，第十五轮用户反馈）

- 用户反馈与处置（排查先行）：① 「更新请求过于频繁」——`app-update.ts` 对 HTTP 403/429（服务端/CDN 限流）的提示文案，改「更新服务限流（请求过于频繁），请稍后再试」更明确；② 多位数字 ID 搜索仍出现「1」且卡顿——API 实测 `11809` 在 v1.16.8 已只命中 `Åntinomiε`（2ms，正确），「1」单数字此前走模糊路径撞上谱师名「譜面-100号」子串（探针 947 分），卡顿源是 `scoreText` 窗口 Levenshtein 对短词的高命中。修复：`getMusicSearchScore` 纯数字查询改**四级规则**（`^\d+$` 含单数字）：ID 精确=1 > 标题/别名精确=0.95（《39》纯数字曲名由此找回）> ID 前缀=0.85（≥2 位）> 2~3 位数字为 ID 子串=0.5；`trimmed` 先 `normalize('NFKC')`（全角数字→半角）；`scoreText` 对 ≤2 字符查询禁用容错窗口；`matchesMusic` 经 `getMusicSearchScore` 短路谱面级评分。四级规则下搜「39」能命中《39》曲（0.95），搜「1」只出 ID/标题精确命中、不再出谱师噪音；③ covers 曲绘缓存统计——曲绘分 `thumbs/`+`full/` 两子目录后，`measureDirectoryWithFileApi` 只数一级 File 恒 0。修复：改为**递归**统计，`measureCovers` 分列 `thumbCount`/`fullCount`，`StorageBreakdown` 增 `coverThumbCount`/`coverFullCount`，设置页曲绘缓存行显示「缩略图 X 张 · 原图 Y 张」；④ 战绩 RA（B50）——`snapshots.tsx` 与 `BattleReportShareCard` 删「（B50）」后缀，只显示 `+N RA`；⑤ FCP→FC+——新增 `status-labels.ts` `formatClearStatus`（fcp→FC+、app→AP+、fsd→FDX+ 等），歌曲详情、计划卡片、分享卡三处统一替换原始 fc/fs 字符串；⑥ AP50——新 `src/data/ap50.ts` `computeAp50`（只收 fc∈{ap,app} 官方 AP/AP+，全库单榜 50，官方公式 RA），`RatingGrid` 增 `ap50ToUnified` 与 `RatingGridCell hideAchievementBar`，新 `Ap50ShareCard`，`app/b50.tsx` 双模式扩三段（B50 总览/拟合 50/AP50），列表/网格均未满补占位，网格完成率颜色条隐藏；⑦ 完成率损失表——`AchievementLossCard` 音符类型列移出横向 ScrollView（`tableWrap` flexRow + `typeColumn` 固定 92 宽），只滚动判定数据列；⑧ 搜索进度——`music-store.applyFiltersChunked` 让步由 `setImmediate` 改 `setTimeout(0)`（宏任务保证 UI 帧刷新），FilterBar 在分片前 `await setTimeout(0)` 先提交「搜索中」帧，搜索按钮 searching 时显示 `ActivityIndicator`；⑨ 搜索框占位——缩短「搜索曲名 / 曲师 / 谱师」+ `numberOfLines={1}` 不再被挤压换行；⑩ 识别匹配——逐条实测（真数据探针）单字符差异（`α→a`、`0→O`）在 v1.16.8 已能匹配（0.81），「完全正确也被屏蔽」真根因是**噪音过滤**的 `titleContains` 豁免要求行长 ≥3，纯数字短曲名《39》恒被过滤。修复（`title-search.ts`）：豁免改为「行与任一真实曲名**完全相等**直接放行（`titleEquals`，取消长度门槛）」；`scoreTitle` 增**滑动窗口**（行内找标题±2 长度、距离 ≤2 的窗口，距离 1 给 0.78+ / 距离 2 给 0.70+，解决粘连行单字符差异）；**子串 0.96 规则只对 ≥4 字符标题**，短标题子串命中降档 0.6（防《39》在粘连行 0.96 霸榜）；探针固化为正式回归脚本 `scripts/title-match-check.ts`（15 用例，`npm run test:match`）。更新日志浮层补 1.16.9 条目。
- 回归与版本：八组门禁全绿（lint/test:rating/test:phase-af/test:features/test:backup/test:update/test:routes/test:match）；`expo-doctor` 21/21；`expo export --platform android` 通过；`app.json` 升 `1.16.9`/versionCode `35`。发布补记（当日）：功能提交 `163c250`（22 文件 +638/−65）；轻量标签 `v1.16.9` git 推送一次成功。云构建 run `33386715251` 一次通过。Release 为 <https://github.com/FengLingYaaa/maimate-app/releases/tag/v1.16.9>；资产 `MaiMate-latest.apk` 大小 `62,367,524` bytes（59.5 MB），SHA-256 `d72e768a582e5292d60f910ba65f79cc63c1da930cff00220724c927c6508577`（GitHub digest 与本地下载资产复算一致）。落地页更新 v1.16.9 文案、日志与 SHA 后经 wrangler 部署 Pages（deployment `8ec2a03f.maimate-landing.pages.dev`）；线上校验落地页含 v1.16.9 与新 SHA、GitHub 直链 200 / content-length `62,367,524` 与资产一致。SHA/大小回填为文档收尾提交。等待用户真机验收（AP50 榜单与分享卡、识别《39》/粘连行/单字符差异、纯数字搜索四级规则与「39」可搜到、搜索进度条即时出现、损失表固定列、曲绘缓存缩略图/原图张数、成绩状态 FC+/AP+、战报 RA 后缀删除）。


### v1.17.0（2026-09-04，第十六轮用户反馈）

- 用户反馈与处置：① 抽歌「不含已达标」计数错误——实测 22 首（4 达标/18 未达标）却显示 20 张：根因是 `random.tsx` 的 `achievedPlanIds` 用 `entry.musicType || 'SD'` 匹配成绩，而计划页用真实 `row.music.type`；DX 曲若条目 musicType 缺失则漏判达标、又因 `planCandidates` 按 id 回退联表成功而照常入池，统计偏差。修复：新增共享 `computeAchievedIds(entries, scores, rawData)`（按真实 SD/DX type、建成绩索引、先 id+type 精确匹配再按 id 回退），random 与 plan 两处共用同一判定口径，消除漏判；② 信息查询页 B50 描述补 AP50（`explore.tsx`）；③④ AP50 汇总显示「AP（含 AP+）N / AP+ M」——`computeAp50` 新增 `totalCount`/`apPlusCount` 字段，b50 汇总卡与 `Ap50ShareCard` 同步展示三项数据；AP50 空位占位符由「虚位」统一为「-」（与分享卡一致）；⑤ 清除搜索进度条残留——`FilterBar.clearSearch` 补复位 `searchProgress`/`searchDoneText`/`searching`；⑥ 曲库默认不加载全曲——`index.tsx` 新增 `libraryLoaded` 态，首屏仅搜索/筛选栏 + 居中「加载全曲」按钮（空态），点击 `loadAllSongs` 或任意搜索/筛选（`handleApply`）后加载列表；⑦ 搜索提速——`getMusicSearchScore` 对每首曲的标题/别名/曲师/谱师反复 `normalizeSearchText`（NFKC+正则）是主要耗时；新增按曲对象身份的 `searchFieldCache`（WeakMap）预归一化去重缓存，`scoreNormalized` 复用已归一化文本，`test:match` 回归保持不变；⑧ 推分战报排序——原按 `ratingDelta`（单曲 RA）而与显示口径 `b50Delta` 不一致；改为「对 B50 有加分（b50Delta>0）置顶 → 按 b50Delta 降序 → 按 ratingDelta 降序」，removed（b50Delta null）沉底（排序符号曾写反，经探针修正）；⑨ 计划页「全部/已达标/未达标」筛选从进度环下方移入 `progressSummaryRow` 右侧（info 弹性右侧留白），删除孤立 `b50Row`。更新日志浮层补 1.17.0 条目。feature-check 增补 3 组用例（computeAchievedIds 真实 type、ap50 计数、战报排序）。
- 回归与版本：八组门禁全绿（lint/test:rating/test:phase-af/test:features/test:backup/test:update/test:routes/test:match）；`expo-doctor` 21/21；`expo export --platform android` 通过；`app.json` 升 `1.17.0`/versionCode `36`。发布补记（当日）：功能提交 `58377fe`（16 文件 +243/−71）；`main` push 遭遇 GitHub:443 间歇阻断（api.github.com 通），重试多次后成功；轻量标签 `v1.17.0` 推送一次成功。云构建 run `33418518753` 一次通过。Release 为 <https://github.com/FengLingYaaa/maimate-app/releases/tag/v1.17.0>；资产 `MaiMate-latest.apk` 大小 `62,369,696` bytes（59.5 MB），SHA-256 `1db51ea671cf538b0f1f1bc220ca516cf77e480622693ce6f2ad7cb2f98634d6`（GitHub digest 与本地下载资产复算一致）。落地页更新 v1.17.0 文案、日志与 SHA 后经 wrangler 部署 Pages（deployment `da1e2d57.maimate-landing.pages.dev`）；线上校验落地页含 v1.17.0 与新 SHA、GitHub 直链 200 / content-length `62,369,696` 与资产一致。等待用户真机验收（抽歌「不含已达标」计数=18、曲库首屏空态+加载全曲、搜索手感提速、AP50 三项计数、战报加分曲置顶、计划筛选进进度环右侧、清搜索进度条消失）。


### v1.17.1（2026-09-05，第十七轮用户反馈）

- 用户反馈与处置：① 曲库与推分计划新增「成绩高 → 低」排序——按选中难度的本机达成率排序，无成绩排末尾，排序难度未指定时默认 Master（`app/(tabs)/index.tsx`、`app/(tabs)/plan.tsx`、`app/(tabs)/settings/sort.tsx`、`src/store/music-store.ts`、`src/components/FilterBar.tsx`）；② 成绩 / 官方定数 / 拟合定数排序支持指定难度，未选时默认 Master，曲库卡片在选中难度旁标注本机成绩（`src/components/SongCard.tsx`、`src/data/music-list.ts`）；③ P1/P2 排序重建修复——推分计划条目解析与排序维度按真实谱面重建（`src/data/plan-entries.ts`：新增按真实成绩所在谱面的 SD/DX 类型解析索引，`chartKeyOf` 以 `(songId, 真实type, 难度)` 为成绩键、WeakMap 按数组身份缓存索引），计划卡片新增「排序成绩」行，条目难度与排序难度不同时也能一眼看到用于排序的本机达成率（`src/components/PlanEntryCard.tsx`、`PlanDragList.tsx`）；④ 抽歌「已达目标」判定修复——计划条目 SD/DX 类型缺失或写错时按真实成绩所在谱面解析，达标计数不再偏大（`app/(tabs)/random.tsx` 与计划共用同一 `plan-entries` 解析口径）；⑤ AP50 统计拆成「AP」「AP+」两项分别展示，详情页与分享卡同步（`app/b50.tsx`、`src/components/Ap50ShareCard.tsx`）；⑥ B50 / AP50 页面去掉「服务器 RA」干扰信息，聚焦本页数据；⑦ 完成率损失卡更新「每 2550 落 ≈ N Tap（Great）」等效容错提示（P·2550 判定档，`src/components/AchievementLossCard.tsx`）；⑧ 完成率损失表格新增分割线并统一行高，判定列滚动更易读；⑨ 云构建/下载站改为版本化 APK 文件名 `MaiMate-v1.17.1.apk` 与 `MaiMate-latest.apk` 并存（`.github/workflows/build-apk.yml`、`docs/BUILD.md`），便于区分历史版本。更新日志浮层补 1.17.1 条目（`src/data/changelog.ts`）；feature-check 增补成绩排序与真实谱面解析用例（`scripts/feature-check.ts`）。
- 回归与版本：八组门禁全绿（lint / test:routes / test:features / test:phase-af / test:rating / test:backup / test:update / test:match）；`expo-doctor` 21/21；`expo export --platform android` 通过；`app.json` 升 `1.17.1`/versionCode `37`。发布补记（当日）：功能提交 `2e37da31c4f58f45a24d45855e9220a59fec826b`（25 文件 +742/−227）；附注标签 `v1.17.1` 已推送。云构建 run `33950192118` 一次成功。Release 为 <https://github.com/FengLingYaaa/maimate-app/releases/tag/v1.17.1>；资产 `MaiMate-v1.17.1.apk` 与 `MaiMate-latest.apk` 均为 `62,375,776` bytes（59.5 MB），SHA-256 `82175e12004f505a731c8521378a5259fbbd32cc52b259d8b890b723a3e2a7a1`（GitHub digest 与本地下载资产复算一致）。落地页更新 v1.17.1 文案、日志与 SHA 后经 wrangler 部署 Pages（deployment `8b68ae6a.maimate-landing.pages.dev`，deployment ID `8b68ae6a-d652-49a1-80fe-9d72cbf90ee0`，production）；线上校验：落地页 <https://maimate.flya.ccwu.cc/> 200 且含 v1.17.1 与新 SHA、下载按钮 latest 直链保留，版本化 Release URL 与 latest URL 均 302→200 / content-length `62,375,776` 与资产一致（latest 资产完整下载复算 SHA 与版本化资产一致）。等待用户真机验收（成绩排序与默认 Master、计划卡「排序成绩」行、抽歌达标计数不再偏大、AP50 双项统计、B50/AP50 去服务器 RA、每 2550 落等效容错、损失表对齐、版本化 APK 直链与 latest 并存）。