...
| 当前分支 | `main` |
| 当前提交 | v1.16.6 功能主体（前一代 `044b81c` 为 v1.16.5 文档收尾） |
| 当前标签 | v1.16.6 已推送（触发云构建，构建结果见发布补记） |
| Android applicationId | `cc.flya.maimate` |
| App 版本 | `1.16.6`（Android versionCode `32`） |
...
- 回归与版本：七组门禁全绿；`expo-doctor` 21/21；`expo export --platform android` 通过；`app.json` 升 `1.16.6`/versionCode `32`。发布补记待构建后回填。
